...
export const MovieDetail = () => {
  ...
  useEffect(()=>{
    const fetchMovie = async ()=>{
      const response= await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=fb83108cad3d591712cb684e129f8732`)
    ...
    <main>
      <section className="flex justify-around flex-wrap my-10">
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white" >
          <h1 className="text-4xl text-bold my-3 text-center lg:text-left">{movie.title}</h1>
          <p className="text-left my-4">{movie.overview}</p>
